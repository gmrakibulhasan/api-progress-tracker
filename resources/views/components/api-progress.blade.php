                    ...
                    <template x-for="api in group.apis" :key="api.id">
                        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            ...
                            <div class="flex items-center space-x-3">
                                ...
                                <!-- Priority Dropdown -->
                                <select x-model="api.priority" 
                                        @change="updateApiPriority(api)"
                                        class="px-2 py-1 text-xs border border-gray-300 rounded">
                                    <option value="low">Low</option>
                                    <option value="medium">Medium</option>
                                    <option value="high">High</option>
...
function apiProgressData() {
    return {
        ...
        async updateApiStatus(api) {
            try {
                const response = await fetch(`{{ route("apipt.api.progress.store") }}`.replace('/api-progress', `/api-progress/${api.id}`), {
                    method: 'PUT',
                    headers: {
        ...
        async updateApiPriority(api) {
            try {
                const response = await fetch(`{{ route("apipt.api.progress.store") }}`.replace('/api-progress', `/api-progress/${api.id}`), {
                    ...
                    body: JSON.stringify({ priority: api.priority })
                });
